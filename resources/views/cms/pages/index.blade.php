...
@foreach ($pages as $page)
    ...
    <div><a href="{{ route('pages.edit', $page->id) }}" class="inline-block bg-lime-700 hover:bg-lime-600 text-white px-4 py-2 rounded">Edit</a></div>
    <div><confirm-delete href="{{ route('pages.destroy', $page->id) }}" title="{{ $page->title }}" csrf="{{ csrf_token() }}">Delete</confirm-delete></div>
@endforeach
</div>

<div class="my-4"><a href="{{ route('pages.create') }}" class="inline-block bg-lime-700 hover:bg-lime-600 text-white px-4 py-2 rounded">Add page</a></div>
